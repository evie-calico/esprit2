...
impl<'texture> ServerHandle<'texture> {
	pub fn new<'lua>(
		...
		sender.queue(&ClientPacket::Authenticate(authentication));
		sender.queue(&ClientPacket::Route(routing));
	...
	pub fn perform_action(
		...
		self.sender
			.queue(&protocol::ClientPacket::Action(action.clone()));
		...
		match input::controllable_character(
			...
			Ok((mode, response)) => match response {
				...
				Some(input::Response::Act(action)) => {
					self.perform_action(scripts, action).unwrap();
					mode
				}
